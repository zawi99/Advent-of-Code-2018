...
guards_sleep_time_minutes = defaultdict(lambda: defaultdict(int))  # if key doesn't exist the defaultdict(int) will be
# called
...
for line in data:
    ...
    if 'begins shift' in line:
        ...
        for minute in range(falls_asleep_time, wakes_up_time):
            ...
            guards_sleep_time_minutes[guard_id][minute] += 1  # how long guard slept at the particular minute
...
best_guard = max_key_by_value(guards_sleep_time)
best_min = max_key_by_value(guards_sleep_time_minutes[best_guard])
print(f'Part 1 result: {best_guard * best_min}')
